...
def dessiner_graphe(canvas, current_tab):
    ...
    for index, (s1, s2, orientee) in enumerate(aretes):
        ...
        if orientee:
            canvas.create_line(x1, y1, x2, y2, arrow=tk.LAST, width=2, tags="arete")
        else:
...
def trouver_chemins():
    ...
    # Demander les sommets de départ et d'arrivée
    def submit():
        start = int(start_entry.get()) - 1  # Convertir en index
        end = int(end_entry.get()) - 1  # Convertir en index
        
        if start < 0 or start >= n or end < 0 or end >= n:
            messagebox.showerror("Erreur", "Sommets invalides. Veuillez entrer des sommets valides.")
            return

        chemins = []
        trouver_chemins_util(matrice_adj, start, end, [start], chemins)

        # Afficher les chemins dans une nouvelle fenêtre
        chemins_window = tk.Toplevel(fenetre)
        chemins_window.title("Chemins Trouvés")
        chemins_label = tk.Label(chemins_window, text="\n".join([" -> ".join([str(v + 1) for v in chemin]) for chemin in chemins]), font=("Helvetica", 12))
        chemins_label.pack(padx=10, pady=10)

    chemin_window = tk.Toplevel(fenetre)
    chemin_window.title("Trouver Chemins")
    
    tk.Label(chemin_window, text="Sommet de départ:").pack(padx=10, pady=5)
    start_entry = tk.Entry(chemin_window)
    start_entry.pack(padx=10, pady=5)

    tk.Label(chemin_window, text="Sommet d'arrivée:").pack(padx=10, pady=5)
    end_entry = tk.Entry(chemin_window)
    end_entry.pack(padx=10, pady=5)

    submit_button = tk.Button(chemin_window, text="Trouver Chemins", command=submit)
    submit_button.pack(pady=10)

def trouver_chemins_util(matrice_adj, current, end, path, chemins):
    if current == end:
        chemins.append(path.copy())
        return

    for voisin in range(len(matrice_adj)):
        if matrice_adj[current][voisin] == 1 and voisin not in path:  # Vérifier si l'arête existe et si le sommet n'est pas déjà dans le chemin
            path.append(voisin)
            trouver_chemins_util(matrice_adj, voisin, end, path, chemins)
            path.pop()  # Retour en arrière
...
def parcours_largeur():
    ...
    canvas = tk.Canvas(frame, width=800, height=600, bg='white', yscrollcommand=v_scroll.set)
...
def dijkstra(matrice_adj, poids, sommet_depart):
    n = len(matrice_adj)
    distances = {i: float('inf') for i in range(n)}
    distances[sommet_depart] = 0
    predecesseurs = {i: None for i in range(n)}
    visites = set()

    while len(visites) < n:
        # Trouver le sommet avec la distance minimale
        sommet_courant = min((i for i in range(n) if i not in visites), key=lambda x: distances[x])

        visites.add(sommet_courant)

        for voisin in range(n):
            if matrice_adj[sommet_courant][voisin] == 1 and voisin not in visites:
                distance = distances[sommet_courant] + poids.get((sommet_courant, voisin), float('inf'))
...
# Fonction pour saisir les poids des arêtes et exécuter l'algorithme de Dijkstra
def algorithme_dijkstra():
    current_tab = notebook.nametowidget(notebook.select())
    data = tab_data[str(current_tab)]
    sommets = data['sommets']
    aretes = list(data['aretes'])
    ...
    for index, (s1, s2, orientee) in enumerate(aretes):
        matrice_adj[s1][s2] = 1
        if not orientee:
            matrice_adj[s2][s1] = 1  # Ajout de la connexion dans les deux sens pour les arêtes non orientées

    # Créer une nouvelle fenêtre pour saisir les poids des arêtes
    poids_window = tk.Toplevel(fenetre)
    poids_window.title("Saisir les poids des arêtes")

    # Créer des champs de saisie pour chaque arête
    for index, (s1, s2, orientee) in enumerate(aretes):
        label = tk.Label(poids_window, text=f"Poids de l'arête entre {s1 + 1} et {s2 + 1}:")
        label.grid(row=index, column=0, padx=10, pady=5)

        entry = tk.Entry(poids_window)
        entry.grid(row=index, column=1, padx=10, pady=5)
        entry.insert(0, "0")  # Valeur par défaut

        poids[(s1, s2)] = entry  # Stocker le champ d'entrée par arête
        if not orientee:
            poids[(s2, s1)] = entry  # Pour les arêtes non orientées, stocker aussi l'autre sens

    def valider_poids():
        for (s1, s2) in poids.keys():
            # Vérifier si le poids est un champ d'entrée ou un float
            if isinstance(poids[(s1, s2)], tk.Entry):
                poids_arrete = poids[(s1, s2)].get()
                try:
                    poids_arrete = float(poids_arrete)
                    if poids_arrete < 0:
                        raise ValueError("Le poids doit être positif.")
                    poids[(s1, s2)] = poids_arrete  # Stocker le poids comme float
                    if not orientee:
                        poids[(s2, s1)] = poids_arrete  # Pour les arêtes non orientées
                except ValueError:
                    tk.messagebox.showerror("Erreur", "Veuillez entrer des poids valides.")
                    return
            else:
                # Si c'est déjà un float, on ne fait rien
                continue

        # Exécuter l'algorithme de Dijkstra
        chemins = dijkstra(matrice_adj, poids, 0)  # Commencer depuis le sommet 0

        # Afficher les résultats
        afficher_resultats(chemins)

        poids_window.destroy()  # Fermer la fenêtre après validation

    # Bouton pour valider les poids
    submit_button = tk.Button(poids_window, text="Valider les poids", command=valider_poids)
    submit_button.grid(row=len(aretes), columnspan=2, pady=10)

# Fonction pour afficher les résultats de Dijkstra
def afficher_resultats(chemins):
    distances, predecesseurs = chemins
    resultats_window = tk.Toplevel(fenetre)
    resultats_window.title("Résultats de l'Algorithme de Dijkstra")
    
    resultats_label = tk.Label(resultats_window, text="Distances minimales depuis le sommet 1:", font=("Helvetica", 12))
    resultats_label.pack(padx=10, pady=10)

    for sommet, distance in distances.items():
        result_label = tk.Label(resultats_window, text=f"Sommet {sommet + 1}: {distance}", font=("Helvetica", 10))
        result_label.pack(padx=10, pady=2)

    # Optionnel : afficher le chemin le plus court
    chemin_label = tk.Label(resultats_window, text="Chemins les plus courts:", font=("Helvetica", 12))
    chemin_label.pack(padx=10, pady=10)

    for sommet in range(len(distances)):
        chemin = []
        current = sommet
        while current is not None:
            chemin.append(current + 1)  # Ajouter 1 pour l'affichage
            current = predecesseurs[current]
        chemin.reverse()
        chemin_str = " -> ".join(map(str, chemin))
        chemin_result_label = tk.Label(resultats_window, text=f"Sommet {sommet + 1}: {chemin_str}", font=("Helvetica", 10))
        chemin_result_label.pack(padx=10, pady=2)

    # Mettre à jour le graphique avec les poids
    mettre_a_jour_graphique_avec_poids(distances)

# Fonction pour mettre à jour le graphique avec les poids
def mettre_a_jour_graphique_avec_poids(distances):
    current_tab = notebook.nametowidget(notebook.select())
    canvas = current_tab.winfo_children()[0]  # Récupérer le canvas
    data = tab_data[str(current_tab)]
    sommets = data['sommets']
    aretes = list(data['aretes'])

    # Effacer les poids précédents
    canvas.delete("poids")

    # Dessiner les poids sur le graphique
    for (s1, s2) in distances.keys():
        x1, y1 = sommets[s1]
        x2, y2 = sommets[s2]
        mx, my = (x1 + x2) / 2, (y1 + y2) / 2  # Point médian de l'arête
        poids = distances[(s1, s2)]
        canvas.create_text(mx, my, text=str(poids), fill="red", tags="poids")
...
chemin_menu.add_command(label='Trouver Chemins', command=trouver_chemins)
affichage_menu.add_cascade(label='Chemin', menu=chemin_menu)

# Exécution
execution_menu = tk.Menu(mon_menu, tearoff=0)
execution_menu.add_command(label="Parcours en largeur", command=parcours_largeur)
execution_menu.add_command(label="Algorithme de Dijkstra", command=algorithme_dijkstra)
mon_menu.add_cascade(label="Exécution", menu=execution_menu)

# Edition
edition_menu = tk.Menu(mon_menu, tearoff=0)
mon_menu.add_cascade(label="Edition")
